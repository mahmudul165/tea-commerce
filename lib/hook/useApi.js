...
const useCollectionQuery = (collection, url, refetchInterval, staleTime, cacheTime) =>
  useQuery([collection], async () => {
    try {
      const response = await axios.get(url);
      return response.data;
...
export const useCarrierCollectionQuery = () =>
  useCollectionQuery("CarrierCollection", CARRIER_ENDPOINT, 15000, 15000, 180000);

export const useGalleryCollectionQuery = () =>
  useCollectionQuery("GalleryCollection", GALLERY_ENDPOINT, 20000, 20000, 240000);

export const useBuninessCollectionQuery = () =>
  useCollectionQuery("BusinessCollection", BUSINESS_ENDPOINT, 30000, 30000, 360000);
